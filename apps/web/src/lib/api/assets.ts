...
// Per-stage status for derived artifacts. `pending` until the worker picks up
// the asset-metadata job; `ready` when the artifact landed in MinIO; `failed`
// when the worker gave up; `not_applicable` for asset types that don't produce
// the artifact (e.g. image → no waveform, audio → no proxy/thumb).
export type DerivedStatus = 'pending' | 'ready' | 'failed' | 'not_applicable';
...
export interface Asset {
  ...
  // Per-stage ingest status. The editor uses `transcriptStatus` to decide when
  // Viona has enough context to start planning — it holds the first prompt
  // until every transcribable asset is `ready` or `failed`.
  thumbnailStatus?: DerivedStatus;
  waveformStatus?: DerivedStatus;
  proxyStatus?: DerivedStatus;
  transcriptStatus?: DerivedStatus;
  ...
  url?: string;
  /**
   * Presigned URL to a low-res H.264 proxy MP4 (~480p, CRF 28). Generated by
   * the asset-metadata worker; null when the asset has no proxy yet (image,
   * audio-only, or proxy generation failed / still pending). The editor
   * should prefer this URL for preview playback — streaming the original
   * full-quality source on every seek is wasteful.
   */
