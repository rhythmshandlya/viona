...
import { KeyframeToggle } from '../../properties/KeyframeToggle';
import { useEditorStore } from '../../../store/use-editor-store';
import type { TimelineItem, Transform } from '../../../store/types';
...
export function TransformSection({ item }: TransformSectionProps) {
  ...
  const renderRow = (label: string, prop: TransformKey, opts: { min?: number; max?: number; step?: number; unit?: string }) => (
    <div className="flex items-center gap-2">
      <span className="text-xs w-16 shrink-0 text-[var(--editor-text-muted)]">
        {label}
      </span>
      <NumberInput
