...
const FILTER_CONFIGS: FilterConfig[] = [
  { key: 'brightness', label: 'Brightness', unit: '%', min: 0, max: 200, step: 1, defaultValue: 1, toDisplay: (v) => v * 100, toStore: (v) => v / 100 },
  { key: 'contrast', label: 'Contrast', unit: '%', min: 0, max: 200, step: 1, defaultValue: 1, toDisplay: (v) => v * 100, toStore: (v) => v / 100 },
  { key: 'saturation', label: 'Saturation', unit: '%', min: 0, max: 200, step: 1, defaultValue: 1, toDisplay: (v) => v * 100, toStore: (v) => v / 100 },
  { key: 'blur', label: 'Blur', unit: 'px', min: 0, max: 50, step: 0.5, defaultValue: 0, toDisplay: (v) => v, toStore: (v) => v },
  { key: 'hue', label: 'Hue Rotate', unit: '\u00B0', min: -180, max: 180, step: 1, defaultValue: 0, toDisplay: (v) => v, toStore: (v) => v },
  { key: 'grayscale', label: 'Grayscale', unit: '%', min: 0, max: 100, step: 1, defaultValue: 0, toDisplay: (v) => v * 100, toStore: (v) => v / 100 },
  { key: 'sepia', label: 'Sepia', unit: '%', min: 0, max: 100, step: 1, defaultValue: 0, toDisplay: (v) => v * 100, toStore: (v) => v / 100 },
];

export function FiltersSection({ item }: FiltersSectionProps) {
        ...
        return (
          <div key={cfg.key} className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
              <span className="text-xs text-[var(--editor-text-muted)]">{cfg.label}</span>
              {isNonDefault && (
                <button
                  className="text-[10px] px-1.5 py-0.5 rounded transition-colors text-[var(--editor-text-muted)] bg-[var(--editor-bg-elevated)]"
