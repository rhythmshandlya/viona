...
export class DockerSandboxProvider implements SandboxProvider {
  ...
  async destroy(sandbox: Pick<Sandbox, 'id' | 'volumeId' | 'projectId'>): Promise<void> {
    ...
    // C4 fix: Delete the workspace volume after backup to prevent accumulation
    try {
...
export async function sandboxRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/projects/:id/sandbox', { preHandler: [authMiddleware] }, async (request, reply) => {
    ...
    return withProjectMutex(projectId, async () => {
      ...
      // Upsert DB record — persist internalUrl AND agentUrl so we can
      // reconstruct URLs after API restart (I6 fix)
      const sessionData = {
        ...
        // Store agentUrl in metadata (avoids schema migration)
  ...
  fastify.post('/projects/:id/sandbox/prompt', { preHandler: [authMiddleware] }, async (request, reply) => {
    ...
    // Get agent URL from DB metadata (not provider in-memory state)
  ...
  fastify.post('/internal/sandbox/:id/ready', async (request, reply) => {
    ...
    // Broadcast WebSocket event to project subscribers
  ...
  // C3 fix: Save the FULL manifest (tracks, items, duration, videoSettings),
  // not just videoSettings. Mirrors the existing syncManifestToDb pattern.
  fastify.post('/internal/sandbox/:id/checkpoint', async (request, reply) => {
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        // Update project videoSettings + durationMs
        ...
        // Delete existing tracks (items cascade-delete via FK)
...
async function suspendSandbox(projectId: string): Promise<void> {
  await withProjectMutex(projectId, async () => {
    ...
    try {
      ...
      // Pass DB-stored metadata to provider (no in-memory Map dependency)
      const sandboxMeta = {
      ...
      // Destroy sandbox (also cleans up workspace volume — C4 fix)
...
export class RailwaySandboxProvider implements SandboxProvider {
  ...
  async destroy(sandbox: Pick<Sandbox, 'id' | 'volumeId' | 'projectId'>): Promise<void> {
    ...
    // C4 fix: Delete the volume to prevent accumulation (backup already taken)
    if (sandbox.volumeId) {
      try {
...
### Task 27: Add crash recovery on API restart

**Files:**
- Modify: `packages/api/src/sandbox/routes.ts`
- Modify: `packages/api/src/index.ts`

- [ ] **Step 1: Add rehydrateActiveSandboxes function to routes.ts**

Add the following function to `packages/api/src/sandbox/routes.ts` (exported):
...
- [ ] **Step 2: Call rehydrate on startup**

In `packages/api/src/index.ts`, after registering sandbox routes, add:

```typescript
// Rehydrate active sandboxes after restart
import { rehydrateActiveSandboxes } from './sandbox/routes.js';
...
git commit -m "feat(sandbox): add crash recovery — rehydrate sandbox state on API restart"
```
