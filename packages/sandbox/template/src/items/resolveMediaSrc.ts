...
export function resolveMediaSrc(
  ...
  // In preview mode, prefer proxy variant
  if (!isRendering) {
    const proxyKey = deriveProxyKey(src);
    if (proxyKey) {
      // Proxy in assets map (MinIO URL) — fastest
      if (assets[proxyKey]) return assets[proxyKey];
      // Proxy via staticFile for video/audio (matte files excluded from assets but proxies exist on disk).
      // Skip images — proxy webp files aren't generated by the pipeline.
      if (/\.(mp4|webm|aac|mp3|wav|m4a)$/i.test(proxyKey)) {
        return staticFile(proxyKey);
      }
    }
  }
