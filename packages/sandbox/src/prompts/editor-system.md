...
/workspace/
  ...
  scenes.json                # Scene plan with sections and treatments
```
...
| `mcp__manifest__split_video` | Split a video item at a timestamp — creates two items with adjusted startFrom |
...
```
Tool: mcp__manifest__split_video
...
The `split_video` tool handles this automatically for the split itself, but manual timing adjustments after ripple shifts may require recalculating startFrom.
...
|---|------|-----------|--------|
| 1 | split_video | itemId: abc, atMs: 8200 | originalId: abc, newId: def |
...
- The approved `scenes.json` plan with sections and treatments
...
1. **Read the manifest and plan**: Load the current manifest and `/workspace/scenes.json`.
...
3. Split at zoom-in point:
   ```
   mcp__manifest__split_video { itemId: "<video-item>", atMs: <startMs> }
   → { originalId, newId }
   ```
...
5. Split at zoom-out point:
   ```
   mcp__manifest__split_video { itemId: "<newId>", atMs: <endMs> }
...
- Updated scenes.json with any revisions from verification rounds
...
- If `mcp__manifest__split_video` fails (item not found, atMs out of range), log the error in the trim report and skip that edit. Do not abort the entire phase.
