...
const ProcessFlow: React.FC<ProcessFlowProps> = (props) => {
  ...
  const NODE_LEFT = s(140);
  const CONTENT_LEFT = s(220);
  const TOP_START = s(140);
  const STEP_SPACING = (s(880) - TOP_START) / stepCount;
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: theme.bg, opacity: introOpacity * outroOpacity }}>
      {/* Title */}
      <div style={{ position: 'absolute', top: s(40), left: 0, right: 0, textAlign: 'center', opacity: titleOpacity, transform: `translateY(${titleSlide}px)` }}>
          ...
          <React.Fragment key={i}>
            ...
            {!isLast && (
              <div style={{
                ...
                top: y + NODE_SIZE + s(4),
                width: s(3),
                height: (STEP_SPACING - NODE_SIZE - s(8)) * lineProgress,
            ...
            <div style={{
              ...
              boxShadow: `0 0 ${s(16)}px ${props.accentColor}40`,
            }}>
              <span style={{ fontFamily: FONTS.headline, fontSize: s(24), fontWeight: 800, color: '#FFFFFF' }}>
                {isLast ? '\u2713' : String(i + 1)}
              </span>
            </div>
            ...
            <div style={{
              ...
              <span style={{ fontFamily: FONTS.headline, fontSize: s(26), fontWeight: 700, color: theme.text }}>{step.title}</span>
              {step.description && (
                <div style={{ marginTop: s(6) }}>
                  <span style={{ fontFamily: FONTS.body, fontSize: s(17), fontWeight: 400, color: theme.textMuted, lineHeight: 1.4 }}>{step.description}</span>
                </div>
              )}
            </div>
          </React.Fragment>
