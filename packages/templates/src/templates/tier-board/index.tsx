...
const TierBoard: React.FC<TierBoardProps> = (props) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: theme.bg, opacity: introOpacity * outroOpacity }}>
      {/* Title */}
      <div style={{ position: 'absolute', top: 30, left: 0, right: 0, textAlign: 'center', opacity: titleOpacity }}>
      ...
      {TIERS.map((tier, rowIdx) => {
        ...
        return (
          <div key={tier} style={{ position: 'absolute', left: ROW_LEFT, right: ROW_RIGHT, top: y, height: ROW_HEIGHT - 12, display: 'flex', opacity: rowOpacity, transform: `translateX(${rowSlideX}px)` }}>
            {/* Tier label */}
            <div style={{
              width: LABEL_WIDTH,
              height: '100%',
              ...
              <span style={{ fontFamily: FONTS.headline, fontSize: 48, fontWeight: 900, color: '#0B0F1A' }}>{tier}</span>
            </div>
              ...
              {tierItems[tier].map((name, idx) => {
                ...
                return (
                  <div key={idx} style={{
                    ...
                    <span style={{ fontFamily: FONTS.body, fontSize: 18, fontWeight: 600, color: theme.text, whiteSpace: 'nowrap' }}>{name}</span>
                  </div>
                );
              })}
            </div>
