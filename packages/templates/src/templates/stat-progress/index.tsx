...
const ProgressRing: React.FC<{
  ...
  headlineFont: string;
  bodyFont: string;
}> = ({ value, label, color, progress, textColor, textMuted, headlineFont, bodyFont }) => {
    ...
    <AbsoluteFill style={{ ...bgStyle, opacity: introOpacity * outroOpacity, overflow: 'hidden' }}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: 20 }}>
          ...
          {/* 2×2 ring grid */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32, flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            {rows.map((row, rowIdx) => (
              <div key={rowIdx} style={{ display: 'flex', gap: 64, justifyContent: 'center' }}>
                {row.map((item, colIdx) => {
