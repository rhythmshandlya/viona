...
const LogoStinger: React.FC<LogoStingerProps> = (props) => {
  ...
  if (props.style === 'minimal') {
    ...
    const lineWidth = interpolate(frame, [70, 110], [0, 120], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
    ...
    return (
      <AbsoluteFill style={{ backgroundColor: theme.bg, opacity: outroOpacity }}>
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
          <span style={{ fontFamily: FONTS.headline, fontSize: 80, fontWeight: 800, color: theme.text, letterSpacing: 12, opacity: nameOpacity, transform: `scale(${nameScale})` }}>{props.brandName}</span>
          <div style={{ width: lineWidth, height: 2, backgroundColor: props.accentColor, borderRadius: 1 }} />
          <span style={{ fontFamily: FONTS.body, fontSize: 22, fontWeight: 400, color: theme.textMuted, letterSpacing: 4, opacity: tagOpacity }}>{props.tagline}</span>
        </div>
      </AbsoluteFill>
    );
  }
    ...
    return (
      <AbsoluteFill style={{ backgroundColor: theme.bg, opacity: outroOpacity }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24 }}>
          <span style={{ fontFamily: FONTS.headline, fontSize: 100, fontWeight: 900, color: theme.text, letterSpacing: 8, opacity: nameOpacity, transform: `scale(${nameDisplayScale})` }}>{props.brandName}</span>
          <span style={{ fontFamily: FONTS.body, fontSize: 22, fontWeight: 400, color: theme.textMuted, letterSpacing: 4, opacity: tagOpacity, transform: `translateY(${tagSlide}px)` }}>{props.tagline}</span>
        </div>
      </AbsoluteFill>
    );
  }
  ...
  const shapes = [
    { type: 'circle', startX: -200, startY: -200, startRot: -45 },
    { type: 'square', startX: 1280, startY: -200, startRot: 45 },
    { type: 'circle', startX: 1280, startY: 1280, startRot: 135 },
    { type: 'diamond', startX: -200, startY: 1280, startRot: -135 },
  ];
  ...
  // Accent line
  const lineWidth = interpolate(frame, [160, 200], [0, 80], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
        ...
        return (
          <div key={i} style={{
            ...
            {shape.type === 'circle' ? (
              ...
              <div style={{ width: size, height: size, borderRadius: shape.type === 'diamond' ? 4 : 8, border: `2px solid ${i % 2 === 0 ? props.secondaryColor : props.accentColor}` }} />
            )}
          </div>
        );
      })}
