...
import { db, assets, assetProjectLinks } from '../db/index.js';
...
// Mirror of `packages/api/src/services/arrangement-input-builder.ts`. Gathers
// everything the arrangement agent needs for a project: the first-pass prompt
// (first user chat message), linked assets, and any derived transcripts.
// Forward-compatible sockets (visualAnalyses, sceneBoundaries, ...) are empty
// today; the prompt template references them conditionally.

/**
 * Lists non-deleted assets linked to a project, newest first. Inlined mirror
 * of `listProjectAssets` from the API — the worker only needs the read path.
 */
